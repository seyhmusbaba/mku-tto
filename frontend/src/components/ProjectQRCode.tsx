...
export function ProjectQRCode({ projectId, projectTitle }: Props) {
  ...
  const qrUrl = `${base}/projects/${projectId}/qr?token=${token}`;
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 text-center max-w-xs w-full"
            ...
            <div className="rounded-xl overflow-hidden border mb-4" style={{ borderColor: '#e8e4dc' }}>
              <img
                src={`${base}/projects/${projectId}/qr`}
                alt="QR Kod"
                className="w-full"
                onError={e => { (e.target as HTMLImageElement).src = `https://api.qrserver.com/v1/create-qr-code/?size=256x256&data=${encodeURIComponent(typeof window !== 'undefined' ? window.location.origin + '/projects/' + projectId : '')}`; }}
              />
            </div>
            <p className="text-xs text-muted mb-3">QR kodu okutarak projeye doğrudan erişilebilir</p>
            <div className="flex gap-2">
              <a href={`${base}/projects/${projectId}/qr`} download="proje-qr.png"
