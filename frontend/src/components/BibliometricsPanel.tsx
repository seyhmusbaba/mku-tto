...
export function BibliometricsPanel({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Veri kaynakları ve şeffaflık paneli */}
      <DataSourcePanel mode={mode} user={user} sourceCoverage={sourceCoverage} sampleSize={data?.sampleSize} institutionalData={mode === 'institutional' ? data : null} />
...
/* ─── Veri Kaynakları ve Şeffaflık Paneli ─── */
function DataSourcePanel({ mode, user, sourceCoverage, sampleSize, institutionalData }: {
  mode: Mode;
  user?: any;
  ...
  // Hangi kaynak kaç yayın getirdi (coverage array'i)
  const coverageEntries = Object.entries(sourceCoverage || {})
    .map(([k, v]) => ({ source: k, label: SOURCE_LABELS[k] || k, count: v, color: SOURCE_COLORS[k] || '#94a3b8' }))
    .sort((a, b) => b.count - a.count);

  return (
    <div className="card p-0 overflow-hidden">
      <button onClick={() => setOpen(o => !o)}
        className="w-full p-4 flex items-center justify-between text-left transition-colors hover:bg-[#faf8f4]"
        style={{ background: open ? '#faf8f4' : 'white' }}>
        <div className="flex items-center gap-3">
          <Icon name="info" className="w-4 h-4 text-navy" />
          <div>
            <p className="text-sm font-semibold text-navy">Veri Kaynakları ve Hesaplama Yöntemi</p>
            <p className="text-xs text-muted mt-0.5">Bu panel hangi ID'lerden çekildi, her metrik nereden hesaplandı — şeffaflık</p>
          </div>
        </div>
        <svg className="w-4 h-4 text-muted transition-transform" style={{ transform: open ? 'rotate(180deg)' : 'none' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      {open && (
        <div className="p-5 space-y-4 border-t" style={{ borderColor: '#e8e4dc' }}>
          {/* Kullanıcı ID'leri — researcher modu */}
          {mode === 'researcher' && user && (
            <div>
              <h5 className="text-xs font-semibold text-navy mb-2 uppercase tracking-wide">Kullanılan Akademik ID'ler</h5>
              <div className="space-y-1.5 text-xs">
                <IdRow label="ORCID" value={user.orcidId} note={user.orcidId ? 'Crossref + OpenAlex yazar aramasında kullanıldı — tam kapsam' : 'Tanımlı değil — ana veri kaynağı eksik'} good={!!user.orcidId} />
                <IdRow label="Scopus Author ID" value={user.scopusAuthorId} note={user.scopusAuthorId ? 'Scopus aramasında kullanılır (API anahtarı varsa)' : 'Tanımlı değil — Scopus atıf verileri alınamıyor'} good={!!user.scopusAuthorId} />
                <IdRow label="WoS Researcher ID" value={user.wosResearcherId} note={user.wosResearcherId ? 'Web of Science aramasında kullanılır (API anahtarı varsa)' : 'Tanımlı değil'} good={!!user.wosResearcherId} />
                <IdRow label="Google Scholar ID" value={(user as any).googleScholarId} note={'⚠ Google Scholar resmi API yok — ID kaydedilse bile yayın çekilmez. Sadece profilde bağlantı olarak görünür. Yayın için ORCID gerekir.'} good={false} warn={!!(user as any).googleScholarId} />
              </div>
            </div>
          )}

          {/* Sample kapsama - kurumsal modda */}
          {mode === 'institutional' && institutionalData && (
            <div>
              <h5 className="text-xs font-semibold text-navy mb-2 uppercase tracking-wide">Kurumsal Veri Kapsamı</h5>
              <div className="space-y-1 text-xs text-muted">
                <p>• <strong className="text-navy">Kurumsal toplamlar</strong> (yayın, atıf, h-index, i10): OpenAlex institution endpoint — tüm yayın havuzundan direkt</p>
                <p>• <strong className="text-navy">Örneklem</strong>: En çok atıf alan <strong>{sampleSize || 500}</strong> yayın — FWCI, Top 1%, Q1-Q4, ülke dağılımı bu örneklemden hesaplanır</p>
                <p>• <strong className="text-navy">OA oranı</strong>: {institutionalData.openAccessSource === 'institutional' ? 'Kurum geneli (OpenAlex counts_by_year)' : institutionalData.openAccessSource === 'period-institutional' ? 'Dönem filtreli (counts_by_year)' : 'Örneklemden'}</p>
                {institutionalData.isPeriodFiltered && (
                  <p>• <strong className="text-navy">Dönem filtresi</strong>: {institutionalData.periodLabel}</p>
                )}
              </div>
            </div>
          )}
          ...
          {coverageEntries.length > 0 && (
            <div>
              <h5 className="text-xs font-semibold text-navy mb-2 uppercase tracking-wide">Kaynakların Getirdiği Yayın Sayısı</h5>
              <div className="flex flex-wrap gap-2">
                {coverageEntries.map(e => (
                  <span key={e.source} className="text-xs px-2.5 py-1 rounded-lg font-semibold"
                    style={{ background: e.color + '18', color: e.color, border: `1px solid ${e.color}44` }}>
                    {e.label}: <strong>{e.count}</strong>
                  </span>
                ))}
              </div>
              <p className="text-[11px] text-muted mt-2">DOI bazlı dedupe — aynı yayın birden fazla kaynaktan gelse bile 1 kere sayılır.</p>
            </div>
          )}

          {/* Her metriğin kaynağı */}
          <div>
            <h5 className="text-xs font-semibold text-navy mb-2 uppercase tracking-wide">Her Metrik Nereden Geliyor</h5>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
              <MetricSource label="Toplam Yayın" src="OpenAlex + TR Dizin + Crossref (DOI dedupe)" />
              <MetricSource label="Toplam Atıf" src="Tüm kaynakların maksimumu (her DOI için)" />
              <MetricSource label="h-index / i10-index" src={mode === 'institutional' ? 'OpenAlex institution summary_stats' : 'Yayın atıf sıralamasından hesaplanır'} />
              <MetricSource label="FWCI" src="OpenAlex cited_by_percentile_year — alan-yıl normalize" />
              <MetricSource label="Top 1% / 10%" src="OpenAlex cited_by_percentile_year.max ≥ 99/90" />
              <MetricSource label="Dergi Kalitesi (Q1-Q4)" src="SCImago SJR (ISSN eşleşmesi) + OpenAlex venue fallback" />
              <MetricSource label="Açık Erişim" src="Unpaywall + DOAJ + OpenAlex open_access" />
              <MetricSource label="SDG Eşleştirmesi" src="OpenAlex sustainable_development_goals (≥0.3 skor)" />
              <MetricSource label="Uluslararası Ortaklık" src="OpenAlex authorships.institutions.country_code" />
              <MetricSource label="Yayın Türü" src="OpenAlex type + TR Dizin docType" />
              <MetricSource label="Türkçe Yayınlar" src="TR Dizin (TÜBİTAK ULAKBİM) API — OpenAlex boşluğunu kapatır" />
              <MetricSource label="Patent Verisi" src="EPO OPS (API anahtarı varsa)" />
            </div>
          </div>

          {/* Kullanılmayan servisler */}
          <div style={{ borderTop: '1px solid #e8e4dc', paddingTop: 10 }}>
            <h5 className="text-xs font-semibold text-navy mb-2 uppercase tracking-wide">Kullanılmayan Kaynaklar</h5>
            <ul className="text-xs text-muted space-y-1 pl-4 list-disc">
              <li><strong className="text-navy">Google Scholar</strong>: Resmi API yok. ID kaydedilse bile otomatik çekim yapılmıyor. Sadece profil linki olarak görünür. SerpAPI gibi ücretli servisle entegre edilebilir (~$50/ay).</li>
              <li><strong className="text-navy">YÖK Akademik</strong>: Cloud IP'lerden WAF blokluyor — sadece link. YÖKSİS SOAP servisi resmi yol (IP whitelist gerekir).</li>
              <li><strong className="text-navy">YÖKSİS</strong>: <code>YOKSIS_USERNAME</code> ve <code>YOKSIS_PASSWORD</code> env ayarlanırsa aktifleşir (IP tescili YÖK başvurusu).</li>
            </ul>
          </div>
        </div>
      )}
...
function IdRow({ label, value, note, good, warn }: { label: string; value?: string; note: string; good: boolean; warn?: boolean }) {
  const bg = warn ? '#fffbeb' : good ? '#f0fdf4' : '#fef2f2';
  const border = warn ? '#fde68a' : good ? '#bbf7d0' : '#fecaca';
  const text = warn ? '#92400e' : good ? '#166534' : '#991b1b';
  const icon = warn ? '⚠' : good ? '✓' : '✕';
  return (
    <div className="flex items-start gap-3 p-2 rounded-lg" style={{ background: bg, border: `1px solid ${border}` }}>
      <span className="font-bold" style={{ color: text }}>{icon}</span>
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-navy">{label} {value && <span className="font-mono text-[11px] text-muted">({value})</span>}</p>
        <p className="text-[11px]" style={{ color: text }}>{note}</p>
      </div>
    </div>
  );
}

function MetricSource({ label, src }: { label: string; src: string }) {
  return (
    <div className="flex items-start gap-2 p-1.5">
      <span className="text-navy font-semibold flex-shrink-0">{label}:</span>
