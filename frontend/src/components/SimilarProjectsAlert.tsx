...
export function SimilarProjectsAlert({ title, description, excludeId, type }: Props) {
  ...
  // Son aranan başlığı takip et — sadece başlık değişince yeniden ara
  ...
  useEffect(() => {
    if (title.length < 5) {
      setLocalSimilar([]);
      setAnalysis(null);
      lastSearchedTitle.current = '';
      return;
    }

    // Aynı başlık zaten arandıysa tekrar arama
    if (title === lastSearchedTitle.current) return;

    if (timerRef.current) clearTimeout(timerRef.current);
    timerRef.current = setTimeout(async () => {
      lastSearchedTitle.current = title;
      setLoading(true);
      setAnalysis(null);
      setOpen(true);
...
SADECE aşağıdaki JSON formatında yanıt ver, başka hiçbir şey yazma:
{
  ...
  }, [title]); // ← SADECE title değişince tetikle
