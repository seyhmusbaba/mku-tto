  ...
  return (
    <div className="px-6 py-4 flex items-center justify-between border-t" style={{ borderColor: '#f0ede8' }}>
      ...
      <div className="flex gap-2">
        <button disabled={page <= 1} onClick={() => setPage((p: number) => p - 1)} className="btn-secondary text-xs px-3 py-1.5 disabled:opacity-40">← Önceki</button>
        <span className="btn-ghost text-xs px-3 py-1.5">{page}/{result.totalPages}</span>
        <button disabled={page >= result.totalPages} onClick={() => setPage((p: number) => p + 1)} className="btn-secondary text-xs px-3 py-1.5 disabled:opacity-40">Sonraki →</button>
      </div>
    </div>
  );
}
