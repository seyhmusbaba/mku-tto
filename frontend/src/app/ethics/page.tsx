...
const S: Record<string, any> = {
  pending:      { label: 'Bekliyor',    color: '#d97706', bg: '#fffbeb', icon: '⏳' },
  approved:     { label: 'Onaylandi',  color: '#059669', bg: '#f0fdf4', icon: '✅' },
  rejected:     { label: 'Reddedildi', color: '#dc2626', bg: '#fef2f2', icon: '❌' },
  not_required: { label: 'Gerekmiyor', color: '#6b7280', bg: '#f9fafb', icon: '-' },
};

export default function EthicsPage() {
  const { user } = useAuth();
  const canDecide = user?.role?.name?.toLowerCase().includes('etik') || user?.role?.name === 'Süper Admin';
  ...
  const submit = async () => {
    if (!form.note.trim()) { toast.error('Gecikce zorunlu'); return; }
    setSaving(true);
    try {
      await api.put('/ethics/decision/' + modal.id, form);
      toast.success(form.decision === 'approved' ? 'Onaylandi' : 'Reddedildi');
      setModal(null);
      load();
  ...
  return (
    <DashboardLayout>
      <Header title="Etik Kurul Yönetimi" subtitle="YZ tarafindan analiz edilen projeler" />
      <div className="p-6 space-y-5">
        ...
        {loading ? (
          ...
          <div className="empty-state py-16">
            ...
            <p className="text-sm font-medium text-navy">{tab === 'pending' ? 'Bekleyen inceleme yok' : 'Henüz inceleme yok'}</p>
          </div>
        ) : (
                ...
                <div key={rev.id} className="card p-5">
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <Link href={'/projects/' + p?.id} className="font-display font-semibold text-navy hover:underline truncate">
                          {p?.title || 'Proje'}
                        </Link>
                  ...
                  {rev.aiEthicsReason && (
                    <div className="mt-3 p-3 rounded-xl text-xs" style={{ background: '#fffbeb', border: '1px solid #fde68a' }}>
                      <p className="font-semibold text-amber-800 mb-1">🤖 YZ Analizi:</p>
                      <p className="text-amber-700">{rev.aiEthicsReason}</p>
                    </div>
                  )}

                  {rev.reviewNote && (
                    <div className="mt-2 p-3 rounded-xl text-xs" style={{ background: '#f0fdf4', border: '1px solid #86efac' }}>
                      <p className="font-semibold text-green-800">📋 Kurul Karari {rev.approvalNumber ? '(' + rev.approvalNumber + ')' : ''}:</p>
                      <p className="text-green-700 mt-1">{rev.reviewNote}</p>
                      {rev.reviewer && <p className="text-muted mt-1">— {rev.reviewer.title} {rev.reviewer.firstName} {rev.reviewer.lastName}</p>}
                    </div>
                  )}

                  {rev.status === 'pending' && canDecide && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" style={{ border: '1px solid #e8e4dc' }}>
            <div className="p-5 border-b flex justify-between" style={{ borderColor: '#e8e4dc' }}>
              <h2 className="font-display font-semibold text-navy">Etik Kurul Karari</h2>
              <button onClick={() => setModal(null)} className="text-muted text-lg">✕</button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { v: 'approved', l: '✅ Onaylandi', c: '#059669', bg: '#f0fdf4' },
              ...
              {form.decision === 'approved' && (
                <div>
                  <label className="label">Onay Numarasi</label>
